...
simulator <- function(country, year, pop_wra, pop_anaemic, malaria_weight, interventionlist, trials) {
  ...
  df$YLD <- with(
    df,
    mild_anaemia * burden_mild + moderate_anaemia * burden_moderate + severe_anaemia * burden_severe
  )

  # Need to replace the staple_eff_per and staple_coverage with numerical refs so it's not name-dependent
  df$mild_post_1 <- with(
    df,
    mild_anaemia * (1 - (1 - int1_eff) * int1_coverage)
  )

  df$moderate_post_1 <- with(
    df,
    moderate_anaemia * (1 - (1 - int1_eff) * int1_coverage)
  )

  df$severe_post_1 <- with(
    df,
    severe_anaemia * (1 - (1 - int1_eff) * int1_coverage)
  )

  df$YLD_post_1 <- with(
    df,
    mild_post_1 * burden_mild + moderate_post_1 * burden_moderate + severe_post_1 * burden_severe
  )

  df$mild_post_2 <- df$mild_post_1 - malaria_weight * (df$Pr_pregnant * df$mild_post_1 * (1 - malarials_eff) * malarials_coverage)

  df$moderate_post_2 <- df$moderate_post_1 - malaria_weight * (df$Pr_pregnant * df$moderate_post_1 * (1 - malarials_eff) * malarials_coverage)

  df$severe_post_2 <- df$severe_post_1 - malaria_weight * (df$Pr_pregnant * df$severe_post_1 * (1 - malarials_eff) * malarials_coverage)

  df$YLD_post_2 <- with(
    df,
    mild_post_2 * burden_mild + moderate_post_2 * burden_moderate + severe_post_2 * burden_severe
  )
