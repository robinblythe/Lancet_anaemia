...
# # Which WTP threshold to use?
# # NB: "Opp_Upper" corresponds to Pichon-Riviere opportunity cost method (base case)
# # WTP_list <- c("Opp_Upper", "HCFI_Lower", "Near_Infinite", "HCFI_Upper","Opp_Lower")
# WTP_list <- c("HCFI_Lower")
# 
# # Run for each WTP threshold
# for (Threshold in WTP_list) {
#   # Determine coverage settings based on threshold
#   if (Threshold == "HCFI_Lower") {
#     # For HCFI_Lower, run with both zero and original coverages
#     coverage_settings <- list(
#       "zero" = TRUE,
#       "original" = FALSE
#     )
#   } else {
#     # For all other thresholds, only run with original coverages
#     coverage_settings <- list(
#       "original" = FALSE
#     )
#   }
#   
#   # Run simulations for each coverage setting
#   for (coverage_name in names(coverage_settings)) {
#     zero_coverage <- coverage_settings[[coverage_name]]
#     
#     # Create a copy of the coverage data for this run
#     df_coverage <- df_coverage_base
#     
#     # Set coverages to zero if needed
#     if (zero_coverage) {
#       df_coverage[,c(3, 5, 7, 9)] <- 0
#     }
#     
#     # Set up WTP for this threshold
#     WTP <- df_costs_base |>
#       select(
#         location_name,
#         paste0("WTP_", Threshold)
...
#     results <- do.call(rbind, sim_results[[1]])
#     if (Threshold == "Opp_Upper" && coverage_name == "original") {
#       league_tables <- do.call(rbind, sim_results[[2]])
#       write_csv(league_tables, file = "./Sensitivity_analysis/league_tables.csv")
#     }
#     
#     if (nrow(results) > nrow(na.omit(results))) results <- replace_empty(results)
#     
#     # Create appropriate file suffix based on coverage setting
#     coverage_suffix <- ifelse(zero_coverage, "_zero_coverage", "")
#     
#     # Save simulation results
#     write_csv(results, file = paste0("./Sensitivity_analysis/sim_results_", Threshold, coverage_suffix, ".csv"))
#     
...
#       )
#     
#     write_csv(change, file = paste0("./Sensitivity_analysis/targets_", Threshold, coverage_suffix, ".csv"))
#     print(paste("Mean change for", Threshold, ifelse(zero_coverage, "with zero coverage:", "with original coverage:"), mean(change$change_median)))
#     
#     # Close cluster for this run
#     stopCluster(cl)
#   }
# }
