...
set.seed(888)
sim_results <- foreach(j = 1:n, .combine = "comb", .multicombine = TRUE, .init = list(list(), list()),
                       .export = ls(), .packages = c("tidyverse", "EnvStats")) %dopar% {
  
  try(source("1_Run_model.R", local = T))
...
write_csv(results, file = paste0("sim_results_", Threshold, ".csv"))
#write_csv(league_tables, file = "league_tables.csv")
...
saveRDS(change, file = paste0("sim_results_", Threshold, ".rds"))
```
...
```{r}
change <- readRDS("sim_results_Opp_Upper.rds")
