...
# Options: Opp_Lower, Opp_Upper, HCFI_Lower, HCFI_Upper
Threshold = "Lower"
WTP <- df_costs |>
  select(
    location_name,
...
# Create output lists of reductions and costs
final_reduction <- list()
final_cost <- list()

# Repeat model n times
for (j in 1:n) {
  source("./1_Run_model.R")
  ...
  final_reduction[[j]] <- df_final |>
    ...
    full_join(df_2030, 
              by = join_by(rei_name, Anaemia_rate, Pop_wra, Pop_total, Pop_pregnant)) |>
    mutate(
...
```{r}
change <- do.call(rbind, final_reduction)$Pct_change_anaemic
