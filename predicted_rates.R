...
# Advantage of this method is that it does not incorporate time, which has irregular intervals (i.e. 2010, 2015, 2020, 2021)
...
df <- df_raw |>
  ...
  group_by(location_name, year_id, rei_name) |>
  summarise(
    Prevalence_mode = round(sum(val), digits = 0),
    Prevalence_min = round(sum(lower), digits = 0),
    Prevalence_max = round(sum(upper), digits = 0)
  ) |>
  ungroup() |>
  rowwise() |>
  mutate(Prevalence = list(rtri(
    n = n,
    min = Prevalence_min,
    max = Prevalence_max,
  ...
  suppressWarnings() |>
  unnest(cols = Prevalence) |>
  select(location_name, year_id, rei_name, Prevalence_mode, Prevalence_min, Prevalence_max)
